...
describe("DocumensoService", () => {
	...
	describe("calculateFields method", () => {
		...
		it("should handle multiple signers correctly", () => {
			...
			let pointer = 0;
			// groups of 2 fields (SIGNATURE and EMAIL)
			let index = 0;
			fields.forEach((f, i) => {
				if (pointer > 1) {
					pointer = 0;
				...
				const pageY =
					config.BASE_PAGE_Y +
					(!pointer ? 0 : config.FIELD_HEIGHT + config.SPACING);
		...
		it("should position fields correctly for different scenarios", () => {
			...
			expect(signatureField?.pageY).toBe(config.BASE_PAGE_Y); // BASE_PAGE_Y
			expect(emailField?.pageY).toBe(
				config.BASE_PAGE_Y + config.FIELD_HEIGHT + config.SPACING,
			); // BASE_PAGE_Y + FIELD_HEIGHT + 1
		});
	});

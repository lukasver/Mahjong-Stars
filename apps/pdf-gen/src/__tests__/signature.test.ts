...
describe("DocumensoService", () => {
	...
	describe("calculateFields method", () => {
		...
		it.only("should create fields for a single signer on single page", () => {
			const recipients = [
			...
			console.log("🚀 ~ signature.test.ts:90 ~ fields:", fields);
		...
		it("should create fields for approver and signer", () => {
			...
			// Check approver fields (should be on the left side)
			const approverFields = fields.filter((field) => field.recipientId === 1);
			expect(approverFields).toHaveLength(2);
			expect(approverFields[0]).toMatchObject({
				type: "SIGNATURE",
				pageX: 12, // APPROVER_X
				pageY: 88, // BASE_PAGE_Y
			});

			// Check signer fields
			const signerFields = fields.filter((field) => field.recipientId === 2);

...
class SalesController {
	...
	async updateSale(
		...
		try {
			...
			// If token symbol is different than current
			// look if there is one that exists with same name, if yes, update tokenId
			// else, create a new one a update relation.

			// If Payload is trying to update sale tokenSymbol
			if (
				tokenSymbol &&
				(sale?.tokenSymbol !== tokenSymbol ||
					sale?.tokenContractChainId !== tokenContractChainId)
			) {
				// If no chain is passed, we cannot look for the token.
				invariant(tokenContractChainId, "Token contract chain id is required");
				...
				// If found, we need to update the relation
				if (token) {
					updateData.token = {
						connect: {
							id: token.id,
				...
				// Else, create new token and related entities
				if (!token) {
					...
					updateData.token = {
						create: {
							...
							TokensOnBlockchains: {
								create: {
									name: data.tokenName || tokenSymbol,
									decimals: 18,
			...
			// If Payload is trying to update sale currency:
			if (currency && sale?.currency !== currency) {
				updateData.saleCurrency = {
					connect: {
						symbol: currency,
					},
				};

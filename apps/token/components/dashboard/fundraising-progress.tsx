...
import { SaleStatusSchema, Sale } from '@/common/schemas/generated';
...
export function FundraisingProgress({
  ...
  const isFinished =
    ...
    activeSale?.status === SaleStatusSchema.enum.FINISHED;
  ...
  if (isFinished) {
    return (
      ...
      >
        <FinishedCard sale={activeSale}>{children}</FinishedCard>
      </motion.div>
    );
  }

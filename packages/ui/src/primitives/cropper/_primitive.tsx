...
const CropperRoot: React.FC<CropperRootProps> = ({
  ...
  const latestRestrictedOffsetRef = useRef<{ x: number; y: number }>({
    x: offsetX,
    y: offsetY,
  });
  ...
  const calculateCropData = useCallback(
    (
      ...
      const currentOffsetX =
        finalOffsetX !== undefined
          ? finalOffsetX
          : latestRestrictedOffsetRef.current.x;
      const currentOffsetY =
        finalOffsetY !== undefined
          ? finalOffsetY
      ...
      const topLeftOffsetX =
        currentOffsetX + (cropAreaWidth - scaledWrapperWidth) / 2;
      const topLeftOffsetY =
        currentOffsetY + (cropAreaHeight - scaledWrapperHeight) / 2;
  ...
  useEffect(() => {
    if (
      ...
      if (!isInitialSetupDoneRef.current) {
        ...
        if (
          restrictedCurrent.x !== latestRestrictedOffsetRef.current.x ||
          restrictedCurrent.y !== latestRestrictedOffsetRef.current.y
        ) {
...
> = ({ className, ...restProps }) => {
  ...
  return (
    <div style={getImageWrapperStyle()}>
      <img {...imageProps} className={className} {...restProps} />
    </div>
  );
};
...
const CropperCropArea: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  const { cropAreaWidth, cropAreaHeight, getCropAreaProps, getCropAreaStyle } =
    useCropperContext();
